...
export default function Navbar() {
  ...
  return <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/85 backdrop-blur-xl"><div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
    ...
    <nav className="hidden items-center gap-5 lg:flex">{links.filter(([,href]) => session?.user || !href.startsWith('/my-')).map(([label, href]) => <Link key={href} href={href} className="text-sm font-medium text-slate-600 transition hover:text-slate-950">{label}</Link>)}</nav>
  ...
  </div>{open && <div className="border-t border-slate-100 bg-white px-4 py-3 sm:hidden"><nav className="grid gap-1">{links.filter(([,href]) => session?.user || !href.startsWith('/my-')).map(([label, href]) => <Link onClick={() => setOpen(false)} key={href} href={href} className="rounded-lg px-3 py-3 text-sm font-medium hover:bg-slate-50">{label}</Link>)}{session?.user ? <Link href="/profile" className="rounded-lg px-3 py-3 text-sm font-medium hover:bg-slate-50">Профиль</Link> : <div className="mt-2 flex gap-2"><Link href="/auth/signin" className="flex-1 rounded-xl border px-3 py-2 text-center text-sm font-semibold">Войти</Link><Link href="/auth/signup" className="flex-1 rounded-xl bg-slate-900 px-3 py-2 text-center text-sm font-semibold text-white">Регистрация</Link></div>}</nav></div>}</header>;
}
